...
function validateSpec(spec: ClassifierSpec): void {
	...
	if (!Array.isArray(spec.choices) || spec.choices.length === 0) {
		...
		spec.choices.forEach((choice, i) => {
			...
			if (typeof choice.score !== "number" || Number.isNaN(choice.score))
				...
				problems.push(
					`choices[${i}].score must be within 0..1 (got ${choice.score})`,
				);
		});
	}
	...
	if (spec.passThreshold !== undefined) {
		if (
			typeof spec.passThreshold !== "number" ||
			Number.isNaN(spec.passThreshold)
		)
			problems.push("`passThreshold` must be a number");
		else if (spec.passThreshold < 0 || spec.passThreshold > 1)
			problems.push(
				`\`passThreshold\` must be within 0..1 (got ${spec.passThreshold})`,
			);
	}
	...
	if (typeof spec.instructions === "string") {
		...
		for (const [, name] of spec.instructions.matchAll(VAR_PATTERN)) {
			if (!known.has(name))
				problems.push(
					`unknown template var '{{${name}}}' (known: ${[...known].join(", ")})`,
				);
		}
	}
	...
	if (problems.length > 0) {
		throw new Error(
			`Invalid ClassifierSpec '${spec.name || "(unnamed)"}':\n- ${problems.join("\n- ")}`,
		);
	}
}
 ...
 */
function extractChoiceFromText(
	text: string,
	choices: Choice[],
): string | undefined {
 ...
 */
async function runClassifierJudge(
	spec: ClassifierSpec,
	ctx: ScorerContext,
	...
	const result = query({
		...
		options: {
			...
			outputFormat: {
				type: "json_schema",
				schema: buildChoiceSchema(spec.choices),
			},
	...
	if (!sawSuccess) {
		return {
			failure: "error",
			detail: errorSubtype || "judge did not complete",
		};
	}
...
function tryParseJson(
	text: string,
): { choice?: unknown; reasoning?: unknown } | undefined {
...
export class LLMClassifierScorer extends BaseScorer {
	...
	async evaluate(ctx: ScorerContext): Promise<ScorerResult> {
		const threshold =
			this.spec.passThreshold !== undefined
				? { passThreshold: this.spec.passThreshold }
				: {};
		...
		return {
			...
			reason: chosen
				? `${chosen.label} — ${chosen.description}`
				: `unrecognized choice '${choice}'`,
			metadata: {
