...
export class FileScorer extends BaseScorer {
	...
	constructor(readonly spec: FileScorerSpec) {
		super();
	...
	async evaluate({ workingDir }: ScorerContext): Promise<ScorerResult> {
		...
		const fails: string[] = [];
		if (spec.exists != null && (content !== null) !== spec.exists)
			fails.push(`exists=${spec.exists}`);
		if (content !== null) {
			if (spec.contains != null && !content.includes(spec.contains))
				fails.push(`contains "${spec.contains}"`);
			if (spec.matches != null) {
				...
				if (re != null && !re.test(content))
					fails.push(`matches /${spec.matches}/`);
			}
			if (spec.jsonPath != null) {
				try {
					const val = getDotted(JSON.parse(content), spec.jsonPath.path);
					if (val !== spec.jsonPath.equals)
						fails.push(`jsonPath ${spec.jsonPath.path}`);
				} catch {
