...
/** Read a string field from an unknown object, returning "" if absent. */
 ...
 */
function formatToolInvocation(toolName: string, input: unknown): string {
	// Extract the most important parameter for common tools
	let param = "";

	switch (toolName) {
		case "WebFetch":
			param = strField(input, "url");
			return `⏺ Fetch(${param})`;

		case "Read": {
			param = strField(input, "file_path");
		...
		case "WebSearch":
			param = strField(input, "query");
			return `⏺ WebSearch(${param})`;

		case "Task":
			param = strField(input, "description");
			return `⏺ Task(${param})`;

		case "Skill":
			param = strField(input, "skill");
			return `⏺ Skill(${param})`;
		...
		default:
			...
			if (input != null && typeof input === "object") {
				...
				if (firstKey) {
					param = String(
						(input as Record<string, unknown>)[firstKey],
					).substring(0, 50);
				}
			}
...
function formatToolResult(toolName: string, result: unknown): string {
	...
	if (res?.isError || res?.error) {
		...
		const firstText =
			...
			contentArr[0] != null
				? strField(contentArr[0], "text")
				: "";
		const errorMsg = strField(res, "error") || firstText || "Error";
...
export async function runClaudeCodeEval(
	...
	switch (execution.mode) {
		...
		case "parallel":
			...
			break;
		case "parallel-limit":
			if (!execution.concurrency) {
				throw new Error(
					'concurrency is required when mode is "parallel-limit"',
				);
			}
