import { BuildSuccessScorer, runClaudeCodeEval } from "../src";
...
async function main() {
	...
	for (const promptId of promptIds) {
		...
		const avgDuration =
			promptResults.reduce((sum, r) => sum + r.duration, 0) /
	...
	console.log(
		`Overall pass rate: ${(result.aggregateScores._overall.passRate * 100).toFixed(1)}%`,
	);
}
