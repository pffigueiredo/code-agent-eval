...
const validSpec: ClassifierSpec = {
  name: 'llm:demo',
  instructions: 'Did the change do what the task asked?\nTask:\n{{prompt}}\nDiff:\n{{diff}}',
  choices: [
...
describe('LLMClassifierScorer.evaluate', () => {
  test('structured_output with a valid label maps to score + metadata', async () => {
    ...
    const result = await new LLMClassifierScorer(validSpec).evaluate(dummyContext());
  ...
  test('fallback ignores the prose article "a", keeping the real verdict', async () => {
    ...
    queueResult({ result: 'My verdict is C, because it left a call site untouched.' });
    const result = await new LLMClassifierScorer(validSpec).evaluate(dummyContext());
    expect(result.metadata!.choice).toBe('C');
    expect(result.score).toBe(0);
  });
...
describe('LLMClassifierScorer constructor validation', () => {
  ...
  test('throws on out-of-range score', () => {
    expect(bad({ choices: [{ label: 'A', description: 'x', score: 2 }] })).toThrow(/0\.\.1/);
  });

  test('throws on blank description', () => {
    expect(bad({ choices: [{ label: 'A', description: '', score: 1 }] })).toThrow(/description/);
  });

  test('throws on unknown template var', () => {
    expect(bad({ instructions: 'check {{dif}}' })).toThrow(/unknown template var/);
  });
