...
describe('built-in classifiers are well-formed', () => {
  test.each(specs)('%s passes validateSpec (constructs without throwing)', (_name, spec) => {
  ...
  test.each(specs)('%s has unique labels, descriptions, and scores in 0..1', (_name, spec) => {
  ...
  test.each(specs)('%s sets a passThreshold below its top score', (_name, spec) => {
...
describe('built-in classifiers drive LLMClassifierScorer end-to-end', () => {
  test('InstructionFollowing maps a chosen label to its score + description', async () => {
    ...
    const result = await new LLMClassifierScorer(InstructionFollowing).evaluate(dummyContext());
  ...
  test('CodeQuality maps a chosen label to its score + description', async () => {
    ...
    const result = await new LLMClassifierScorer(CodeQuality).evaluate(dummyContext());
    expect(result.score).toBe(1);
    expect(result.metadata!.choice).toBe('A');
    expect(result.metadata!.chosenDescription).toBe('High — clear, idiomatic, easy to follow');
  });

  test('Security maps a chosen label to its score + description', async () => {
    queueResult({ structured: { choice: 'C', reasoning: 'sql injection' } });
    const result = await new LLMClassifierScorer(Security).evaluate(dummyContext());
