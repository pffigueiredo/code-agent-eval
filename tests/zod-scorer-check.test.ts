...
describe("Zod schema with class-based scorers", () => {
	...
	test("SkillPickedUpScorer evaluate works after schema validation", async () => {
		...
		const ctx: ScorerContext = {
			...
			agentOutput: "[]",
			promptId: "test",
			execCommand: async (_opts) => ({ score: 0, reason: "" }),
		};
	...
	test("plain object scorer still passes validation", () => {
		const result = evalConfigSchema.safeParse({
			...baseConfig,
			scorers: [
				{ name: "custom", evaluate: async () => ({ score: 1, reason: "ok" }) },
			],
		});
		expect(result.success).toBe(true);
	});
...
describe("jsonConfigSchema (JSON path)", () => {
	...
	it("rejects unknown top-level key", () => {
		expect(jsonConfigSchema.safeParse({ ...base, iteration: 5 }).success).toBe(
			false,
		);
	});
	...
	it("rejects unknown scorer key", () => {
		expect(
			jsonConfigSchema.safeParse({
				...base,
				scorers: [{ type: "build", nope: 1 }],
			}).success,
		).toBe(false);
	});
	...
	it("rejects command missing name", () => {
		expect(
			jsonConfigSchema.safeParse({
				...base,
				scorers: [{ type: "command", command: "npm" }],
			}).success,
		).toBe(false);
	});
	...
	it("rejects unknown scorer type", () => {
		expect(
			jsonConfigSchema.safeParse({ ...base, scorers: [{ type: "nope" }] })
				.success,
		).toBe(false);
	});
	...
	it("accepts non-empty all/any `of`", () => {
		const r = jsonConfigSchema.safeParse({
			...base,
			scorers: [{ type: "all", of: [{ type: "build" }] }],
		});
		expect(r.success).toBe(true);
	});
});
