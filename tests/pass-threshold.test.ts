...
describe("isScorePassing", () => {
	...
	describe("custom passThreshold", () => {
		test("passThreshold 0.6 → score 0.75 passes", () => {
			expect(isScorePassing(result({ score: 0.75, passThreshold: 0.6 }))).toBe(
				true,
			);
		});

		test("passThreshold 0.6 → score 0.6 passes (>= boundary)", () => {
			expect(isScorePassing(result({ score: 0.6, passThreshold: 0.6 }))).toBe(
				true,
			);
		});

		test("passThreshold 0.6 → score 0.5 fails", () => {
			expect(isScorePassing(result({ score: 0.5, passThreshold: 0.6 }))).toBe(
				false,
			);
		});
	});
